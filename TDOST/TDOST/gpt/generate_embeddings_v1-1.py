...
def generate_embeddings(dataset, model,  args):
    ...
    data_sensor = np.load(os.path.join(npy_path, "{}-x_sensor.npy".format(dataset)), allow_pickle=True)
    data_value = np.load(os.path.join(npy_path,"{}-x_value.npy".format(dataset)), allow_pickle=True)
    data_time = np.load(os.path.join(npy_path,"{}-x_time.npy".format(dataset)), allow_pickle=True)
    labels = np.load(os.path.join(npy_path,"{}-global_labels.npy".format(dataset)), allow_pickle=True)
    ...
    for i in range(labels.shape[0]):
        ...
        for j in range(min(len(sensor_seq), args.context_length)):
            ...
            # print(time_reading, sensor_reading, value_reading)
            ...
            # time_of_day = get_time_period(time_seq[j].hour)
            # day_of_week = time_seq[j].strftime('%A')
            # sensor_location = sensor_map[sensor_reading]["location"]
            # sensor_type = sensor_map[sensor_reading]["type"]
            
            # sensor_key = str((day_of_week, time_of_day, sensor_type, sensor_location,  value_reading))
            ...
            for sent_no, sent in enumerate(list_of_sent):
                ...
                if j == 0:
                    if sent.endswith('.'):
                        sent = sent[:-1]
                    final_sentence = sent + " " + time_sentence
                else:
                    final_sentence = time_sentence +  " " +sent
    ...
    joblib.dump(list(emb_dict.keys()), f"{dataset}_gpt_v1-1_sentences.joblib", )
    
    emb = emb.reshape(emb.shape[0]* emb.shape[1], emb.shape[2], emb.shape[3])
    np.save(os.path.join(npy_path, "{}_embeddings_gpt_v1-1_{}".format(dataset, args.sentence_encoder_name)), emb)
...
if __name__=="__main__":
    ...
    for dataset in args.datasets:
        print(f"Starting encoding for {dataset}")
